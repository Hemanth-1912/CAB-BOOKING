        ...
        <div className="gps-visualizer-card">
          ...
          <div className="route-map-container">
            <div className="map-road-track">
              ...
              <div className="map-node node-pickup" style={{ left: '50%' }}>
                <span className="node-icon">📍</span>
                <span className="node-label">Pickup</span>
              </div>
              
              {/* Dropoff location indicator */}
              <div className="map-node node-dropoff" style={{ left: '95%' }}>
                <span className="node-icon">🏁</span>
                <span className="node-label">Dropoff</span>
              </div>

              {/* Animated Driver / Cab Icon */}
              {booking.status !== 'requested' && booking.status !== 'cancelled' && (
                <div 
                  className={`animated-cab-marker ${booking.status === 'in_progress' ? 'driving' : 'navigating'}`}
                  style={{ left: `${simLocation}%` }}
                >
                  <span className="cab-marker-icon">🚕</span>
                  <span className="cab-marker-label">
                    {booking.status === 'on_the_way' || booking.status === 'accepted' ? 'Driver approaching' : 'You'}
                  </span>
                </div>
              )}
            </div>
